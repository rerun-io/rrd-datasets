"""
Build every layer (base, URDF, odom, cameras, IR, properties) for every episode in one command.

Each layer module owns its conversion and runs on its own (`convert-base` / `convert-urdf` /
`convert-odom` / `convert-cameras` / `convert-ir` / `convert-properties`); this module runs them
in order. The URDF model is parsed once and reused across episodes. Episodes without a head
stereo calibration skip the cameras layer; episodes without wrist IR streams skip the ir layer.
...
def convert_episode(ep: Episode, urdf: UrdfTree) -> list[str]:
    """Write one episode's layers in `LAYERS` order; returns the layers written (cameras and ir may skip)."""
    results = {
